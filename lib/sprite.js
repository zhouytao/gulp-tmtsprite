...
module.exports = function (options) {
  ...
  return through2.obj(function (css, enc, callback) {
    ...
    var pathArray = css.path.split(path.sep);

    var isFind = false;
    var _tmpCssPath = lodash.filter(pathArray, function(n) {
      if (n === opt.cssInDir) {
        isFind = true;
        return false;
      }
    ...
    // 保留源文件的路经
    var cssPath = _tmpCssPath.join(path.sep);
    // 替换CSS
    var _cssPath = _tmpCssPath.join('/');
    _cssPath = _cssPath.substring(0, _cssPath.lastIndexOf('.'));
    ...
    async.waterfall([
      ...
      function (cb) {
        ...
        if (!codelines || codelines.length === 0) {
          ...
          _this.push(new File({
            base: opt.cssOut,
            path: path.join(opt.cssOut, cssPath),
        ...
        async.eachSeries(codelines, function (backgroundCodeLine, eachCb) {
          ...
          // 保留源文件的路经
          var isFind2 = false;
          var _tmpCssPath2 = lodash.filter(css.path.split(path.sep), function(n) {
            if (n === opt.cssInDir) {
              isFind2 = true;
              return true;
            }
          ...
          var absolutePath = path.join(_tmpCssPath2.join(path.sep), relativePath);
        ...
        if (layerInfo.items.length > 0) {
          ...
          lwip.create(layerInfo.width, layerInfo.height, opt.color, function (err, image) {
            ...
            async.eachSeries(layerInfo.items, function (sprite, callback) {
              ...
              //CSS替换
              var code = 'background-image: url("' + opt.spriteOut + _cssPath + '.png");'
          ...
          lwip.create(retinaLayerInfo.width, retinaLayerInfo.height, opt.color, function (err, image) {
            ...
            async.eachSeries(retinaLayerInfo.items, function (sprite, callback) {
              ...
              lodash.each(sprite.meta.className2x, function (item) {
                ...
                retinaCssContent += '{background-image:url("' + opt.spriteOut + _cssPath + '@2x.png");'
          ...
          lwip.create(retinaLayerInfo.width, retinaLayerInfo.height, opt.color, function (err, image) {
            ...
            async.eachSeries(retinaLayerInfo.items, function (sprite, callback) {
              ...
              lodash.each(sprite.meta.className3x, function (item) {
                retinaCssContent += item
                retinaCssContent += '{background-image:url("' + opt.spriteOut + _cssPath + '@3x.png");'
          ...
          function (next) {
            if (image1x) {
              image1x.toBuffer('png', {}, function (err, spriteBuffer) {
                ...
                _this.push(new File({
                  base: opt.spriteOut,
                  path: path.join(opt.spriteOut, path.dirname(cssPath), cssBaseName + '.png'),
          ...
          function (next) {
            if (image2x) {
              image2x.toBuffer('png', {}, function (err, retinaSpriteBuffer) {
                ...
                _this.push(new File({
                  base: opt.spriteOut,
                  path: path.join(opt.spriteOut, path.dirname(cssPath), cssBaseName + '@2x.png'),
          ...
          function (next) {
            if (image3x) {
              image3x.toBuffer('png', {}, function (err, retinaSpriteBuffer) {
                ...
                _this.push(new File({
                  base: opt.spriteOut,
                  path: path.join(opt.spriteOut, path.dirname(cssPath), cssBaseName + '@3x.png'),
                  contents: retinaSpriteBuffer
                }))
